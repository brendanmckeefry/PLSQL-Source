create or replace PACKAGE BODY FT_PK_COST_WRITES AS
  ...
  BEGIN
    ...
    IF APP_CHNG OR AUTH_CHNG THEN
      IF EXPCHA_REC.ISRECOVERY IN (1,11) THEN
       -- AS WE ARE CHANGING AN ITECHG AND THIS IS AN ACCRUAL WE ALWAYS WANT THE EXCFULLYAUTH = 1 SO THAT IT IS PICKED UP 
        UPDATE EXPCHA
        SET EXCFULLYAUTH = OPEN_ACCRUAL
      ...
      UPDATE EXPCHA
      SET ISRECOVERY = CASE WHEN ABS(NVL(EXCCONAMM,0) - NVL(EXCAUTHCONAMM,0)) < 0.01 THEN 11 ELSE 1 END
      WHERE EXPCHA.EXCCHAREC = ITECHG_REC.EXCRECNO
      AND EXISTS (SELECT 1 
                  FROM ACCCLASS
                  WHERE CLARECNO = EXCSENCODE
                  AND CLAACCCSTSUP = CONST.C_RECOVERY);
      COMMIT;
  ...
  BEGIN
    ...
    IF ITECHG_INOUT.EXCRECNO IS NULL THEN
      ...
      EXPCHA_REC.EXCRECOVFROMPL := EXPCHA_INOUT.EXCRECOVFROMPL;
	    EXPCHA_REC.ISRECOVERY := EXPCHA_INOUT.ISRECOVERY;
    ...
    IF EXPCHA_REC.ISRECOVERY IN (1,11) THEN
       -- AS WE ARE WRITING A NEW ITECHG AND THIS IS AN ACCRUAL WE ALWAYS WANT THE EXCFULLYAUTH = 1 SO THAT IT IS PICKED UP 
        EXPCHA_REC.EXCFULLYAUTH := OPEN_ACCRUAL;
    END IF;   
