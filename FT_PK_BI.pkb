    ...
    LOOP
      ...
      BEGIN 
            ...
            OR LASTUSED_REC.OPERATIONTYPE = C_UPDATE THEN
              ...
              USING 
              (SELECT PRCPRDNO, PRCREF1, PRCREF2,PRCREF3,PRCREF4,PRCREF5,PRCREF6,PRCDESCRIPTION,PRCVATCODE,PRCPRDREF,ACTIVE,
    ...
    LOOP
      ...
      BEGIN 
            ...
            OR LASTUSED_REC.OPERATIONTYPE = C_UPDATE THEN
              ...
              USING 
              (SELECT LOGONNO, LOGONNAME,  USERNAME, sysdate as LASTTOUCHEDDATE  FROM LOGONS
    ...
    LOOP
      ...
      BEGIN 
            ...
            OR LASTUSED_REC.OPERATIONTYPE = C_UPDATE THEN
              ...
              USING 
              ( SELECT SMNNO, SMNTYPE,SMNNAME, sysdate as LASTTOUCHEDDATE FROM SMN
              ...
              WHEN MATCHED THEN 
                  UPDATE SET  BI.SMNTYPE = TMPTAB.SMNTYPE, BI.SMNNAME = TMPTAB.SMNNAME, BI.LASTTOUCHEDDATE = TMPTAB.LASTTOUCHEDDATE 
              WHEN NOT MATCHED THEN
                  INSERT(BI.SMNNO, BI.SMNTYPE,BI.SMNNAME, BI.LASTTOUCHEDDATE )
                  VALUES(TMPTAB.SMNNO,  TMPTAB.SMNTYPE, TMPTAB.SMNNAME, TMPTAB.LASTTOUCHEDDATE );
              END;
            END IF;
      ...
      BEGIN 
            ...
            OR LASTUSED_REC.OPERATIONTYPE = C_UPDATE THEN
              ...
              ( SELECT CLARECNO, CLAACCCSTSUP, CLAACCCODE, CLAACTIVE, CLACONTACTNAME, ACCNAME, CASHCUSTOMER, DEFDLTRECNO,
                  ACCADDRESS1, ACCADDRESS2, ACCADDRESS3, ACCADDRESS4, ACCPOSTCODE, CLATELENO, CLAFAXNO,
      ...
      BEGIN 
            ...
            OR LASTUSED_REC.OPERATIONTYPE = C_UPDATE THEN
              ...
              ( SELECT CURNO,CURDESC,CURTRIANGREQ,CURDECIMALPLACES,CURDECIMALPOINT,CURTHOUSANDSEPARATOR,
                       CURSYMBOL,CURSYMBOLATFRONT,CURUNITNUMPICTURE,CUREXTNNUMPICTURE,CURDEFBANRECNO,
    ...
    LOOP
      ...
      BEGIN 
            ...
            OR LASTUSED_REC.OPERATIONTYPE = C_UPDATE THEN
              ...
              USING 
              ( SELECT CSARECNO,CSACSGRECNO,CSADESC,
   ...
   BEGIN
     ...
     BEGIN   
       ...
       USING 
        ...
        nvl(DelHed.DlvRthNo, DelHed.DropRteNo) as Route_Number,    
        ...
        sysdate as LASTTOUCHEDDATE
            ...
            WHEN NOT MATCHED THEN
			      INSERT(BI.DGPDETAILRECNO, BI.DGPHEDRECNO, BI.DGP_EXTRACTION_DATE, BI.TRANSACTION_NUMBER, BI.DELIVERY_DATE, BI.SALES_OFFICE_NUMBER,
                   ...
                   BI.DLVSALTYPE, BI.INTERDEPTFLAG, BI.TRNSALOFFNO, BI.DEPARTMENT, BI.RECEIVEDDATE, BI.LASTTOUCHEDDATE	)
            VALUES(TMPTAB.TRANSACTION_NUMBER, TMPTAB.DGPHEDRECNO, TMPTAB.DGP_EXTRACTION_DATE, TMPTAB.TRANSACTION_NUMBER, TMPTAB.DELIVERY_DATE, TMPTAB.SALES_OFFICE_NUMBER,
                   ...
                   TMPTAB.DLVSALTYPE, TMPTAB.INTERDEPTFLAG, TMPTAB.TRNSALOFFNO, TMPTAB.DEPARTMENT, TMPTAB.RECEIVEDDATE, TMPTAB.LASTTOUCHEDDATE);
   ...
   BEGIN
     ...
     BEGIN   
       ...
       USING 
            ...
            WHEN MATCHED THEN 
                UPDATE SET 
					BI.DGPCHGRECNO = TMPTAB.DGPCHGRECNO, 
					BI.DGPHEDRECNO = TMPTAB.DGPHEDRECNO,  
  ...
  */  
  ...
  BEGIN
    OPEN SYS_CUR FOR SELECT DGPHEAD.DGPHEDRECNO AS UNIQNO1, -1 AS UNIQNO2 FROM DGPHEAD
        WHERE DGPSALOFFNO = 6 and DGPCLOSED = 1 and DGPHEDRECNO > 5777;
