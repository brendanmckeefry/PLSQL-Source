...
CREATE OR REPLACE PACKAGE BODY FT_COSTING AS
    ...
    IF ACRCOSTDLYRATE_REC.ACSRECNO IS NOT NULL THEN
      ...
      WHEN SCHEMA_REC_IN.ACRCHGFOR = CONST.C_FOR_PO THEN
        ...
        WHEN SCHEMA_REC_IN.ACRCHGPER = CONST.C_APP_BOX AND SCHEMA_REC_IN.LHEPAYTYP = CONST.C_ACCOUNTSALE AND SCHEMA_REC_IN.CHARGECLASS = CONST.C_GOODSREBATE THEN
          IF FT_SALES.GET_LOT_SOLD_QTY(SCHEMA_REC_IN.LITITENO) > 0.0 THEN
      ...
      WHEN SCHEMA_REC_IN.ACRCHGFOR = CONST.C_FOR_RETURN AND SCHEMA_REC_IN.LHEPAYTYP <> CONST.C_SELFINV AND SCHEMA_REC_IN.ACRCHGPER = CONST.C_APP_BOX THEN
        FT_RETURNPRICES.GET_RETURN_PRICES(SCHEMA_REC_IN.LITITENO, L_RETQTY, L_RETVALUE);
        IF L_RETQTY > 0 THEN
  ...
  BEGIN
    ...
    ELSIF COST_REC_IN.ICHCHNGDBYUSER = CONST.C_TRUE THEN
      L_CANCHNG := FALSE;
    ELSIF ABS(COST_REC_IN.ICHAUTHAMM) > 0.009 AND COST_REC_IN.CLAACCCSTSUP <> CONST.C_RECOVERY THEN
      L_CANCHNG := FALSE;
  ...
  BEGIN
    ...
    FT_COST_WRITES.UPDATE_EXPCHA(EXPCHA_INOUT);
  END;
  ...
  IS
    ...
    EXPCHAS_TO_WRITE    FT_COST_WRITES.EXPCHA_RECS := FT_COST_WRITES.EXPCHA_RECS();
    ...
    FOR LIT_ITR IN LITRECS_IN.FIRST..LITRECS_IN.LAST LOOP
      ...
      WHILE CTY_ITR IS NOT NULL LOOP
        ...
        THEN
          ...
          ITECHGS_TO_WRITE(L_THISREC).ICHSPETO := FT_COST_WRITES.POCOST_FIXED;
          ...
          ELSE
            IF SCHEMA_RESULTS(CTY_ITR).ACRCTYNO = CONST.CTYGOODS OR SCHEMA_RESULTS(CTY_ITR).GDSSUPISINV = CONST.C_TRUE THEN
              EXPCHAS_TO_WRITE(L_THISREC).EXCSENCODE := SCHEMA_RESULTS(CTY_ITR).LHESENCODE;
            ELSE
              EXPCHAS_TO_WRITE(L_THISREC).EXCSENCODE := SCHEMA_RESULTS(CTY_ITR).ACRINVSUP;
            END IF;
      ...
      WHILE NOT CTY_ITR IS NULL LOOP
        IF NOT SCHEMA_RESULTS.EXISTS(CTY_ITR) THEN
          L_CANCHNG := CAN_CHANGE_PO_COST(LOT_COSTS(CTY_ITR));
          IF L_CANCHNG THEN
            ITECHGS_TO_WRITE.EXTEND(1);
            EXPCHAS_TO_WRITE.EXTEND(1);
      ...
      FOR ITR IN 1..ITECHGS_TO_WRITE.COUNT LOOP
        IF ITECHGS_TO_WRITE(ITR).ICHRECNO IS NOT NULL THEN
          FT_COST_WRITES.UPDATE_ITECHG(ITECHGS_TO_WRITE(ITR), EXPCHAS_TO_WRITE(ITR));
        ELSE
          FT_COST_WRITES.INSERT_ITECHG(ITECHGS_TO_WRITE(ITR), EXPCHAS_TO_WRITE(ITR));
        END IF;
      END LOOP;
      ...
      FOR ITR IN 1..EXPCHAS_TO_WRITE.COUNT LOOP
        ...
        IF CONTAINER_RECS.EXISTS(CTY_ITR) THEN
          FT_COST_WRITES.APPORTION_PO_COST(EXPCHAS_TO_WRITE(ITR).EXCCHAREC, CONTAINER_RECS(CTY_ITR), TRUE, FALSE);
