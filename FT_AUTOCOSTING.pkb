...
CREATE OR REPLACE PACKAGE BODY FT_AUTOCOSTING AS
  ...
  G_SID           INTEGER := FT_SESSION_UTILS.GET_SID;
  ...
  BEGIN 
    FOR INDX IN 1..SQL%BULK_EXCEPTIONS.COUNT LOOP
      BEGIN
        ...
        FT_ERRORS.RAISE_ERROR(FTERR_IN,  PARAMETER_LIST);           
      EXCEPTION
        WHEN OTHERS THEN
  ...
  BEGIN    
    BEGIN
      ...
      COMMIT;
    EXCEPTION
      WHEN FT_ERRNUMS.EXC_DML_ERRORS THEN
        COMMIT;
        LOG_BULK_LOT_ERRORS(LITRECS_IN, FT_ERRNUMS.FT_ENQUEUE_LIT);
      WHEN OTHERS THEN
        ROLLBACK;
  ...
  EXCEPTION
    WHEN FT_ERRNUMS.EXC_DML_ERRORS THEN
      COMMIT;
      LOG_BULK_DPR_ERRORS(DPRRECS_IN, FT_ERRNUMS.FT_ENQUEUE_DPR);
    WHEN OTHERS THEN  
      ROLLBACK;
      FT_ERRORS.LOG_AND_STOP;
  END ENQUEUE_DPRRECS;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN FT_ERRNUMS.EXC_DML_ERRORS THEN
        COMMIT;
        LOG_BULK_LOT_ERRORS(LITRECS_IN, FT_ERRNUMS.FT_PRIORITISE_LIT);
      WHEN OTHERS THEN
        ROLLBACK;
        FT_ERRORS.LOG_AND_STOP;
    END;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN FT_ERRNUMS.EXC_DML_ERRORS THEN
        COMMIT;
        LOG_BULK_DPR_ERRORS(DPRRECS_IN, FT_ERRNUMS.FT_PRIORITISE_DPR);
      WHEN OTHERS THEN
        ROLLBACK;
        FT_ERRORS.LOG_AND_STOP;
    END;
