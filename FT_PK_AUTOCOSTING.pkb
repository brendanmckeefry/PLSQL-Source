CREATE OR REPLACE PACKAGE BODY FT_PK_AUTOCOSTING AS
  ...
  G_CHUNKSIZE     INTEGER := NVL(TO_NUMBER(FT_PK_UTILS.GET_SYSPREF('AUTOCHUNKSIZE')), 1000);
  ...
  IS 
  BEGIN
    ENQUEUE_LITRECS(LITRECS_IN, COSTCHNGTYPE_IN , 0);
  END ENQUEUE_LITRECS;
  
  PROCEDURE ENQUEUE_LITRECS(LITRECS_IN RECORD_NUMBERS, COSTCHNGTYPE_IN AUTOCOSTTYPES.COSTCHNGTYPENO%TYPE, FORMNO_IN FRMNAME.FORMNO%TYPE)
  IS
  BEGIN    
  ...
  IS 
  BEGIN
    ENQUEUE_DPRRECS(DPRRECS_IN, COSTCHNGTYPE_IN , 0);
  END ENQUEUE_DPRRECS;  

  PROCEDURE ENQUEUE_DPRRECS(DPRRECS_IN RECORD_NUMBERS, COSTCHNGTYPE_IN AUTOCOSTTYPES.COSTCHNGTYPENO%TYPE, FORMNO_IN FRMNAME.FORMNO%TYPE)  
  IS
  BEGIN
  ...
  PROCEDURE ENQUEUE_LIT(LITITENO_IN AUTOCOSTSTODO.LITITENO%TYPE, COSTCHNGTYPE_IN AUTOCOSTTYPES.COSTCHNGTYPENO%TYPE, FORMNO_IN FRMNAME.FORMNO%TYPE)
  IS
  BEGIN
    ENQUEUE_LITRECS(RECORD_NUMBERS(LITITENO_IN), COSTCHNGTYPE_IN, FORMNO_IN);
  END ENQUEUE_LIT;
  
  PROCEDURE ENQUEUE_DPR(DPRRECNO_IN AUTOCOSTSTODO.DPRRECNO%TYPE, COSTCHNGTYPE_IN AUTOCOSTTYPES.COSTCHNGTYPENO%TYPE, FORMNO_IN FRMNAME.FORMNO%TYPE )
  IS
  BEGIN
    ENQUEUE_DPRRECS(RECORD_NUMBERS(DPRRECNO_IN), COSTCHNGTYPE_IN, FORMNO_IN);
  END ENQUEUE_DPR;  
  ...
  PROCEDURE ENQUEUE_DPRRECS_AA(DPRRECS_IN T_INTEGER_ARRAY, COSTCHNGTYPE_IN AUTOCOSTTYPES.COSTCHNGTYPENO%TYPE, FORMNO_IN FRMNAME.FORMNO%TYPE)
  AS
     DELPRICEIDS RECORD_NUMBERS := RECORD_NUMBERS();
  BEGIN
    FOR I IN DPRRECS_IN.FIRST..DPRRECS_IN.LAST
    LOOP
      ...
      DELPRICEIDS(I) := DPRRECS_IN(I);
    END LOOP;
    ENQUEUE_DPRRECS(DELPRICEIDS, COSTCHNGTYPE_IN, FORMNO_IN);
    RETURN;
  END ENQUEUE_DPRRECS_AA;
  ...
  IS
    ...
    CHUNKSIZE_DONE      INTEGER := 0;
    NOOFDPRS            INTEGER := 0;
    SALOFFNO_ITR        AUTOCOSTSTODO.SALOFFNO%TYPE;
  ...
  BEGIN   
    ...
    SELECT (SELECT COUNT(*) FROM AUTOCOSTSTODO WHERE AUTOCOSTSTODO.SESSIONNO = G_SID) INTO CHUNKSIZE_DONE FROM DUAL;
  
    SELECT MIN(AUTOCOSTSTODO.SALOFFNO) INTO  SALOFFNO_ITR FROM AUTOCOSTS_PEND INNER JOIN AUTOCOSTSTODO ON AUTOCOSTS_PEND.AUTOCOSTREC = AUTOCOSTSTODO.AUTOCOSTREC WHERE AUTOCOSTSTODO.SESSIONNO IS NULL ;
    
      FOR AUTOCOST_ITR IN(SELECT AUTOCOSTS_PEND.AUTOCOSTREC FROM AUTOCOSTS_PEND INNER JOIN AUTOCOSTSTODO ON AUTOCOSTS_PEND.AUTOCOSTREC = AUTOCOSTSTODO.AUTOCOSTREC WHERE AUTOCOSTSTODO.SESSIONNO IS NULL AND ROWNUM < G_CHUNKSIZE AND AUTOCOSTSTODO.SALOFFNO = SALOFFNO_ITR ORDER BY AUTOCOSTREC) LOOP    
      IF CHUNKSIZE_DONE < G_CHUNKSIZE THEN
      BEGIN          
        -- COUNT THE NUMBER OF DELPRICES ASSOC WITH THIS AUTOCOST LINE
        -- FOR LOTS I ASSUME THAT DPRSTOLOTS HAS A FAIRLY ACCURATE NUMBER AS THIS WILL BE CONTINUOSLY UPDATED
        SELECT (CASE WHEN NVL(DPRRECNO, 0) > 0 
                    THEN 1 
                    ELSE  (SELECT COUNT(DISTINCT DTLDPRRECNO) FROM DPRSTOLOTS WHERE DPRSTOLOTS.DTLLITITENO = AUTOCOSTSTODO.LITITENO)
                END) NODPRRECNO
        INTO NOOFDPRS 
        FROM AUTOCOSTSTODO
        WHERE AUTOCOSTREC = AUTOCOST_ITR.AUTOCOSTREC;
        
        CHUNKSIZE_DONE := CHUNKSIZE_DONE  + NVL(NOOFDPRS,0) ;
        
        IF (CHUNKSIZE_DONE < G_CHUNKSIZE) 
        OR (NVL(NOOFDPRS,0) > G_CHUNKSIZE) THEN  -- IN CASE  WE HAVE A LOT WITH GREATER THAN THE CHUNCK SIZE - IT WILL HAVE TO BE DONE IN ONE HIT REGARDLESS
        BEGIN          
