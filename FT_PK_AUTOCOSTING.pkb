CREATE OR REPLACE PACKAGE BODY FT_PK_AUTOCOSTING AS
  ...
  BEGIN
    ...
    BEGIN
      FORALL INDX IN INDICES OF LITRECS_IN SAVE EXCEPTIONS
        ...
        SELECT AUTOCOSTSTODO.AUTOCOSTREC 
        FROM AUTOCOSTSTODO 
        WHERE AUTOCOSTSTODO.PROCESSSTAT = PROCESSTODO
  ...
  BEGIN   
    ...
    WHERE AUTOCOSTSTODO.SESSIONNO IS NULL
      AND ROWNUM < G_CHUNKSIZE;
  
    UPDATE AUTOCOSTSTODO
    SET AUTOCOSTSTODO.PROCESSSTAT = PROCESSINPROG,
        AUTOCOSTSTODO.SESSIONNO = G_SID
    WHERE AUTOCOSTSTODO.AUTOCOSTREC IN(SELECT AUTOCOSTS_PROCESS.AUTOCOSTREC FROM AUTOCOSTS_PROCESS);  
    COMMIT;                
  EXCEPTION
    WHEN OTHERS THEN
      ROLLBACK;
  ...
  BEGIN
    ...
    SET PROCESSSTAT = PROCESSTODO,
        SESSIONNO = NULL
    WHERE AUTOCOSTREC IN(SELECT DEADSESSIONS.AUTOCOSTREC FROM AUTOCOSTSTODO DEADSESSIONS WHERE DEADSESSIONS.SESSIONNO NOT IN(SELECT * FROM TABLE(ACTIVE_SID)));
  EXCEPTION
    WHEN OTHERS THEN
      FT_PK_ERRORS.LOG_AND_STOP;     
  ...
  BEGIN
    ...
    SELECT	MIN(AUTOCOSTS_PROCESS.AUTOCOSTREC),
            ...
            MAX(AUTOCOSTS_PROCESS.GETSALES)
		FROM AUTOCOSTS_PROCESS
    INNER JOIN DPRSTOLOTS
      ON DPRSTOLOTS.DTLDPRRECNO = AUTOCOSTS_PROCESS.DPRRECNO
		WHERE NOT EXISTS(SELECT * FROM AUTOCOSTS_PROCESS WHERE AUTOCOSTS_PROCESS.LITITENO = DPRSTOLOTS.DTLLITITENO)
