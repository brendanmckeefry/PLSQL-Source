CREATE OR REPLACE PACKAGE BODY FT_PK_AUTOCOSTING AS
  ...
  IS
    ...
    CHUNKSIZE_TODO      INTEGER := G_CHUNKSIZE;
  ...
  BEGIN   
    ...
    SELECT CHUNKSIZE_TODO - (SELECT COUNT(*) FROM AUTOCOSTSTODO WHERE AUTOCOSTSTODO.SESSIONNO = G_SID) INTO CHUNKSIZE_TODO FROM DUAL;
  
    FOR AUTOCOST_ITR IN(SELECT AUTOCOSTS_PEND.AUTOCOSTREC FROM AUTOCOSTS_PEND INNER JOIN AUTOCOSTSTODO ON AUTOCOSTS_PEND.AUTOCOSTREC = AUTOCOSTSTODO.AUTOCOSTREC WHERE AUTOCOSTSTODO.SESSIONNO IS NULL AND ROWNUM < CHUNKSIZE_TODO ORDER BY AUTOCOSTREC) LOOP
