...
CREATE OR REPLACE PACKAGE BODY FT_PK_WO_COSTS AS
  ...
  IS
  BEGIN
    FOR LITREC IN (SELECT AUTOCOSTS_PROCESS.LITITENO FROM AUTOCOSTS_PROCESS WHERE AUTOCOSTS_PROCESS.RECALCULATEWOCOSTS = CONST.C_TRUE) LOOP
      RECALCULATEWOCOSTS(LITREC.LITITENO);
    END LOOP;
  EXCEPTION
    WHEN OTHERS THEN
      FT_PK_ERRORS.LOG_AND_STOP;
