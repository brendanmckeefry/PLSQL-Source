CREATE OR REPLACE PACKAGE BODY FT_PK_GETSALES AS
  ...
  PROCEDURE GETSALES IS
    ...
    NOOFRECS          INTEGER := 0;
  BEGIN  
    SELECT COUNT(*) INTO NOOFRECS FROM AUTOCOSTS_DPRS WHERE AUTOCOSTS_DPRS.GETSALES = 1;
    FOR DPRREC IN (SELECT * FROM AUTOCOSTS_DPRS WHERE AUTOCOSTS_DPRS.GETSALES = 1)LOOP
      DPRRECNOS_IN.EXTEND();
