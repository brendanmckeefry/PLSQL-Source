create or replace PACKAGE BODY FT_PK_GETSALES AS
  ...
  IS
    ...
    CURSOR DPRTOLOTS_CUR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
    SELECT *
    FROM DPRSTOLOTS
    ...
    CURSOR DPRSTOLOTSCHGS_CUR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
    SELECT *
    ...
    CURSOR BULKSALESCURSOR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
    ...
    ,SUM(Case When (Abs(NVL(delprice.delprice, 0)) > 0.009
         ...
         OR NVL(delprice.delfreeofchg, 0) = 1) --exclude openprice
              ...
              0
              END) SOLDQTY
    ,CASE WHEN NVL(delhed.DlvCurRecNo, 1) <> 1 then Round(NVL(delprice.DelToBaseRate,1.00) * SUM(NVL(DELTOIST.DISNETTVALUE,0)), 2) else SUM(NVL(DELTOIST.DISNETTVALUE,0)) end NETTVALUE
    ,SUM(Case When (Abs(NVL(delprice.delprice, 0)) > 0.009
         OR Abs(NVL (delnettvalue, 0)) > 0.009
         OR NVL(delprice.delfreeofchg, 0) = 1) --exclude openprice
              then
    ...
    delhed.DlvSalOffNo
      ...
      GROUP BY  DELPRICE.DPRRECNO, ITESTO.ISTLITNO, DELPRICE.DELPRICE, DELNETTVALUE, DELFREEOFCHG, DPRDELRECNO, DELHED.DLVORDNO, DELHED.DLVSALOFFNO, DELPRICE.DELTOBASERATE, DELHED.DLVCURRECNO
      ORDER BY  DELPRICE.DPRRECNO, ITESTO.ISTLITNO, DELPRICE.DELPRICE, DELNETTVALUE, DELFREEOFCHG, DPRDELRECNO, DELHED.DLVORDNO, DELHED.DLVSALOFFNO, DELPRICE.DELTOBASERATE, DELHED.DLVCURRECNO;

    cursor PREPSALESCURSOR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    is
    select
    DELPRICE.DPRRECNO
   ,ITESTO.ISTLITNO
   ...
   ,SUM (case when (ABS(NVL(DELPRICE.DELPRICE, 0)) > 0.009
    ...
    cursor BULKDPRCHGS(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    is
      select ICHRECNO
      ,DTLRECNO
      ...
      ,case when ABS(DPRSTOLOTS.DTLOPENPRCQTY) > 0.009
            then
              ...
              NVL(ICHAUTHTOGLAMTBASE,0)
            END ICHAUTHTOGLAMTBASE
      ,case when ABS(DPRSTOLOTS.DTLOPENPRCQTY) > 0.009
            then
              0
            else
              NVL(ICHAUTHTOGLAMTRAW,0)
            end ICHAUTHTOGLAMTRAW
    ...
    CURSOR PREPDPRCHGS(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
    SELECT ICHRECNO
    ,DTLRECNO
    ,SUM(NVL(RAWAPPAMTPERBULKLOT,0)) RAWAPPAMT
    ,SUM(NVL(APPAMTPERBULKLOT,0)) ICHAPPAMT
    ,SUM(NVL(ICHAUTHTOGLAMTBASE,0)) ICHAUTHTOGLAMTBASE
    ,SUM(NVL(ICHAUTHTOGLAMTRAW,0)) ICHAUTHTOGLAMTRAW    
    ...
    FROM(SELECT PRECHGS.ICHRECNO
          ...
          ,PRECHGS.ICHAPPAMT
          ,PRECHGS.RAWAPPAMT
          ,PRECHGS.ICHAUTHTOGLAMTBASE
          ,PRECHGS.ICHAUTHTOGLAMTRAW                
          ,PRECHGS.DPRQTYTHIS
          ,PRECHGS.DISQTY
          ,DPRSTOLOTS.DTLRECNO
          ,ROUND(
          ...
          FROM(
                ...
                ,NVL(ICHAUTHTOGLAMTBASE,0) ICHAUTHTOGLAMTBASE
                ,NVL(ICHAUTHTOGLAMTRAW,0) ICHAUTHTOGLAMTRAW
    ...
    CURSOR PREPCHGNULLISTCURSOR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
      ...
      , ROUND(CASE WHEN ABS(PRECHGS.DTLOPENPRCQTY) > 0.009
            ...
            , ROUND(
              ...
              ELSE
                 CASE WHEN NVL(DPRISPRICEADJONLY,0) = 1
                  THEN
                    CASE WHEN ABS(PRECHGS.DELNETTVALUE) > 0 THEN (NVL(ICHAUTHTOGLAMTBASE,0) * ABS(PRECHGS.DTLSALESVALUE)) / ABS(PRECHGS.DELNETTVALUE)  ELSE 0 END
                  ELSE
                    CASE WHEN ABS(PRECHGS.DELPRCQTY) > 0 THEN (NVL(ICHAUTHTOGLAMTBASE,0) * ABS(PRECHGS.DTLSOLDSALESQTY)) / ABS(PRECHGS.DELPRCQTY)  ELSE 0 END
                  END
              END,2) BASEAPPAUTHTOGLAMTPERBULKLOT 
            , ROUND( 
              CASE WHEN ABS(PRECHGS.DTLOPENPRCQTY) > 0.009
              THEN
                 0
              ELSE
                 CASE WHEN NVL(DPRISPRICEADJONLY,0) = 1
                  THEN
                    CASE WHEN ABS(PRECHGS.DELNETTVALUE) > 0 THEN (NVL(ICHAUTHTOGLAMTRAW ,0) * ABS(PRECHGS.DTLSALESVALUE)) / ABS(PRECHGS.DELNETTVALUE)  ELSE 0 END
                  ELSE
                    CASE WHEN ABS(PRECHGS.DELPRCQTY) > 0 THEN (NVL(ICHAUTHTOGLAMTRAW ,0) * ABS(PRECHGS.DTLSOLDSALESQTY)) / ABS(PRECHGS.DELPRCQTY)  ELSE 0 END
                  END
              END,2) RAWAPPAUTHTOGLAMTPERBULKLOT
    ...
    CURSOR PREPSALESCHGNOEXPCUR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
    SELECT PRECHGS.ICHRECNO
     ,PRECHGS.DTLRECNO
     , ROUND(
    ...
    CURSOR SALESRTECHGS(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
    SELECT PRECHGS.ICHRECNO
      ,PRECHGS.DTLRECNO
      , ROUND(
      ...
      , CASE  WHEN NVL(DELQTY,0) > 0
              THEN (NVL(ICHAUTHTOGLAMTBASE ,0) * NVL(CAST(DELPRCQTY AS FLOAT),0)) / CAST(DELQTY AS FLOAT)
              ELSE 0
              END ICHAUTHTOGLAMTBASE 
      , CASE  WHEN NVL(DELQTY,0) > 0
              THEN (NVL(ICHAUTHTOGLAMTRAW,0) * NVL(CAST(DELPRCQTY AS FLOAT),0)) / CAST(DELQTY AS FLOAT)
              ELSE 0
              END ICHAUTHTOGLAMTRAW              
      ,NVL(EXPCHA.EXCRECOVFROMPL,0) EXCRECOVFROMPL
      ,CHGTYP.CTYNO
    ...
    CURSOR DOSALESROUNDINGCUR(DPRRECNO_IN DELPRICE.DPRRECNO%TYPE)
    IS
      SELECT ITECHG.ICHRECNO,ITECHG.ICHAPPAMT, MAX(DPRSTOLOTSCHGS.DTLCHGSRECNO) MAXDTLCHGSRECNO, 
      NVL(ITECHG.ICHAPPAMT,0) - SUM(NVL(DPRSTOLOTSCHGS.DTLCHGSBASEAPP,0)) ROUNDDIFF,
      NVL(ITECHG.ICHAUTHTOGLAMTBASE,0) - SUM(NVL(DPRSTOLOTSCHGS.DTLBASEAUTHTOGLAMT, 0)) ROUNDDIFF_BASEAUTHTOGLAMT,
      NVL(ITECHG.ICHAUTHTOGLAMTRAW,0) - SUM(NVL(DPRSTOLOTSCHGS.DTLRAWAUTHTOGLAMT,0)) ROUNDDIFF_RAWAUTHTOGLAMT      
      ...
      GROUP BY ITECHG.ICHRECNO, ITECHG.ICHAPPAMT, ICHAUTHTOGLAMTBASE, ICHAUTHTOGLAMTRAW, DPRRECNO
  ...
  BEGIN
    ...
    FOR DPRREC IN DPRRECNOS_IN.FIRST..DPRRECNOS_IN.LAST LOOP
      ...
      FOR BULKSALESREC IN BULKSALESCURSOR(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          COUNTSI := COUNTSI + 1;
      ...
      FOR PREPSALESREC IN PREPSALESCURSOR(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          COUNTSI := COUNTSI + 1;
          INSERT INTO DPRSTOLOTS(DTLDPRRECNO,
                                DTLLITITENO,
      ...
      FOR BULKDPRCHGREC IN BULKDPRCHGS(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          ...
          SELECT   BULKDPRCHGREC.ICHRECNO,
                   ...
                   BULKDPRCHGREC.ICHAUTHTOGLAMTBASE,
                   BULKDPRCHGREC.ICHAUTHTOGLAMTRAW,
      ...
      FOR PREPDPRCHGREC IN PREPDPRCHGS(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          INSERT INTO DPRSTOLOTSCHGS( DTLCHGSICHNO,
                                      DTLCHGSDTLRECNO,
                                      DTLCHGSRAWAPP,
          ...
          SELECT PREPDPRCHGREC.ICHRECNO,
                 ...
                 PREPDPRCHGREC.ICHAUTHTOGLAMTBASE,
                 PREPDPRCHGREC.ICHAUTHTOGLAMTRAW,                 
      ...
      FOR PREPCHGNULLISTREC IN PREPCHGNULLISTCURSOR(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          INSERT INTO DPRSTOLOTSCHGS( DTLCHGSICHNO,
                                      DTLCHGSDTLRECNO,
                                      DTLCHGSRAWAPP,
          ...
          SELECT  PREPCHGNULLISTREC.ICHRECNO,
                  ...
                  PREPCHGNULLISTREC.BASEAPPAUTHTOGLAMTPERBULKLOT,
                  PREPCHGNULLISTREC.RAWAPPAUTHTOGLAMTPERBULKLOT,                  
      ...
      FOR PREPSALESCHGNOEXPREC IN PREPSALESCHGNOEXPCUR(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          INSERT INTO DPRSTOLOTSCHGS( DTLCHGSICHNO,
                                      DTLCHGSDTLRECNO,
                                      DTLCHGSRAWAPP,
          ...
          SELECT  PREPSALESCHGNOEXPREC.ICHRECNO,
                  ...
                  PREPSALESCHGNOEXPREC.BASEAPPAUTHTOGLAMTPERBULKLOT,
                  PREPSALESCHGNOEXPREC.RAWAPPAUTHTOGLAMTPERBULKLOT,
      ...
      FOR SALESRTECHGSREC IN SALESRTECHGS(DPRRECNOS_IN(DPRREC)) LOOP
        BEGIN
          INSERT INTO DPRSTOLOTSCHGS( DTLCHGSICHNO,
                                      DTLCHGSDTLRECNO,
                                      DTLCHGSRAWAPP,
          ...
          SELECT  SALESRTECHGSREC.ICHRECNO,
                  ...
                  SALESRTECHGSREC.BASEAPPAUTHTOGLAMTPERBULKLOT,
                  SALESRTECHGSREC.RAWAPPAUTHTOGLAMTPERBULKLOT,
      ...
      FOR DOSALESROUNDINGREC IN DOSALESROUNDINGCUR(DPRRECNOS_IN(DPRREC)) LOOP
        UPDATE DPRSTOLOTSCHGS
        SET DTLCHGSBASEAPP = NVL(DTLCHGSBASEAPP,0)  +  DOSALESROUNDINGREC.ROUNDDIFF,
        DTLBASEAUTHTOGLAMT = NVL(DTLBASEAUTHTOGLAMT,0)  +  DOSALESROUNDINGREC.ROUNDDIFF_BASEAUTHTOGLAMT,
        DTLRAWAUTHTOGLAMT  = NVL(DTLRAWAUTHTOGLAMT,0)  +  DOSALESROUNDINGREC.ROUNDDIFF_RAWAUTHTOGLAMT       
  ...
  IS
    ...
    CURSOR GETBALSTOLOTSCHGSTODEL(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    ...
    CURSOR GETBALSTOLOTSTODEL(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    ...
    CURSOR GETBALSTOLOTSCUR(LITITETO_IN LOTITE.LITITENO%TYPE)
    IS
    SELECT LITITENO
    ,PORSALOFF
    ...
    CURSOR GETTRANSFERQTYCUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    ...
    CURSOR GETTRANSBALSTOLOTSCUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    SELECT ISTLITNO
    ,TRNSALOFFNO
    ...
    CURSOR GETSALEQTYFORBALSCUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    ...
    CURSOR BALSTOLOTSCHGSCUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    SELECT  BALTOLOTS.BTLRECNO,
        ITECHG.ICHRECNO,
        CHGTYP.CTYNO,
        ...
        NVL(ITECHG.ICHAUTHTOGLAMTBASE, 0.0) ICHAUTHTOGLAMTBASE,
        NVL(ITECHG.ICHAUTHTOGLAMTRAW, 0.0) ICHAUTHTOGLAMTRAW
  ...
  BEGIN
    ...
    FOR BALTOLOTSCHGSREC IN GETBALSTOLOTSCHGSTODEL(LITITENO_IN) LOOP
      DELETE FROM BALTOLOTSCHGS
      WHERE BALTOLOTSCHGS.BTLCHGSRECNO = BALTOLOTSCHGSREC.BTLCHGSRECNO;
      COMMIT;
    END LOOP;

    FOR BALTOLOTSCHGSREC IN GETBALSTOLOTSTODEL(LITITENO_IN) LOOP
      DELETE FROM BALTOLOTS
      WHERE BALTOLOTS.BTLRECNO = BALTOLOTSCHGSREC.BTLRECNO;
      COMMIT;
    END LOOP;

    FOR GETBALSTOLOTSREC IN GETBALSTOLOTSCUR(LITITENO_IN) LOOP
      BEGIN
    ...
    FOR GETTRANSFERQTYREC IN GETTRANSFERQTYCUR(LITITENO_IN) LOOP
      UPDATE BALTOLOTS
    ...
    FOR GETTRANSBALSTOLOTSREC IN GETTRANSBALSTOLOTSCUR(LITITENO_IN) LOOP
      BEGIN
        INSERT INTO BALTOLOTS(BTLLITITENO
                             ,BTLSALOFFNO
                             ,RCVQTY)
    ...
    FOR GETSALEQTYFORBALSREC IN GETSALEQTYFORBALSCUR(LITITENO_IN) LOOP
      UPDATE BALTOLOTS
    ...
    FOR  BALSTOLOTSCHGSREC IN BALSTOLOTSCHGSCUR(LITITENO_IN) LOOP
      BEGIN
        ...
        SELECT  BALSTOLOTSCHGSREC.BTLRECNO,
                ...
                BALSTOLOTSCHGSREC.ICHAUTHTOGLAMTBASE,
                BALSTOLOTSCHGSREC.ICHAUTHTOGLAMTRAW
  ...
  IS
    CURSOR GET_ROUNDING_TODO(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    ...
    CURSOR DPRSTOLOTSPURCHGSDEL_CUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    ...
    CURSOR DPRSTOLOTSPURCHG_CUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    SELECT
     ICHRECNO
    ,DTLRECNO
    ...
    ,ROUND(CASE WHEN TOTQTY > 0 THEN (ICHAUTHTOGLAMTBASE * DTLBULKSALESQTY) /  TOTQTY ELSE 0 END, 2) LOTTODPRCHG_AUTHTOGLAMTBASE
    ,ROUND( CASE WHEN TOTQTY > 0 THEN (ICHAUTHTOGLAMTRAW * DTLBULKSALESQTY) /  TOTQTY ELSE 0 END, 2) LOTTODPRCHG_AUTHTOGLAMTRAW
    ,EXCRECOVFROMPL
    ,CTYNO
    ,CHARGECLASS
    ,2 TYPENO
    ...
    CURSOR GETBALTOLOTSPURCHG_CUR(LITITENO_IN LOTITE.LITITENO%TYPE)
    IS
    SELECT BTLCHGSRECNO
    FROM BALTOLOTSCHGS, BALTOLOTS
    WHERE BALTOLOTSCHGS.BTLRECNO = BALTOLOTS.BTLRECNO
  ...
  BEGIN
    ...
    FOR DPRSTOLOTSPURCHGSDELREC IN DPRSTOLOTSPURCHGSDEL_CUR(LITITENO_IN) LOOP
      DELETE FROM DPRSTOLOTSCHGS
      WHERE DPRSTOLOTSCHGS.DTLCHGSRECNO =  DPRSTOLOTSPURCHGSDELREC.DTLCHGSRECNO;
      COMMIT;
    END LOOP;

    FOR DPRSTOLOTSPURCHGREC IN DPRSTOLOTSPURCHG_CUR(LITITENO_IN) LOOP
      BEGIN
        INSERT INTO DPRSTOLOTSCHGS( DTLCHGSICHNO
                                   ,DTLCHGSDTLRECNO
                                   ,DTLCHGSRAWAPP
        ...
        SELECT  DPRSTOLOTSPURCHGREC.ICHRECNO,
                ...
                DPRSTOLOTSPURCHGREC.LOTTODPRCHG_AUTHTOGLAMTBASE,
                DPRSTOLOTSPURCHGREC.LOTTODPRCHG_AUTHTOGLAMTRAW,             
    ...
    FOR GETBALTOLOTSPURCHGREC IN GETBALTOLOTSPURCHG_CUR(LITITENO_IN) LOOP
