CREATE OR REPLACE PACKAGE BODY FT_PK_PROFITISELOTS AS
  ...
  BEGIN
    ...
    UPDATE LOTPROFITSALOFF
    SET FULLYSOLDINVOICED = NVL((SELECT MIN(CASE WHEN ABS(LOTITERCVD.RCVDQTY - LOTITESOLD.SOLDQTY) < 0.01 THEN CONST.C_TRUE ELSE CONST.C_FALSE END)
    ...
    SET POCOSTAUTH =	NVL(( SELECT MIN(CASE
                                        WHEN SALOFFNO.LOTPROFITMETH IN(1, 2) AND ABS(NVL(COST_VIEW.ICHAPPAMT, 0.0) - NVL(COST_VIEW.ICHAUTHTOGLAMTBASE, 0.0)) > 0.009 THEN CONST.C_FALSE
                                        WHEN SALOFFNO.LOTPROFITMETH IN(3, 4) AND ABS(NVL(COST_VIEW.ICHAUTHTOGLAMTBASE, 0.0)) < 0.01 THEN CONST.C_FALSE
                                        ELSE CONST.C_TRUE
                                      END)
    ...
    SET SALESCOSTAUTH = NVL(( SELECT MIN(CASE
                                          WHEN SALOFFNO.LOTPROFITMETH IN(1, 2) AND ABS(NVL(COST_VIEW.ICHAPPAMT, 0.0) - NVL(COST_VIEW.ICHAUTHTOGLAMTBASE, 0.0)) > 0.009 THEN CONST.C_FALSE
                                          WHEN SALOFFNO.LOTPROFITMETH IN(3, 4) AND NVL(COST_VIEW.ICHAUTHTOGLAMTBASE, 0.0) < 0.01 THEN CONST.C_FALSE
                                          ELSE CONST.C_TRUE
                                        END)
    ...
    UPDATE LOTPROFITSALOFF
    SET ISOPENLOT = CASE WHEN REOPENED = CONST.C_TRUE OR PROFITISED = CONST.C_FALSE THEN CONST.C_TRUE ELSE CONST.C_FALSE END
    WHERE NOT EXISTS(SELECT * FROM LOTITE WHERE LOTITE.LITITENO = LOTPROFITSALOFF.LITITENO AND ONRESERVE = CONST.C_TRUE) 
