create or replace PACKAGE BODY FT_PK_SALES AS
  ...
  PROCEDURE DELPRICE_NETTVALUE(IN_DPRRECNO IN NUMBER) AS
    ...
    V_CURNO         DELHED.DLVCURRECNO%TYPE ;
  ...
  BEGIN
    ...
    END;
    BEGIN
      SELECT DELNETTVALUE, DELHED.DLVCURRECNO 
      INTO V_DELNETTVALUE, V_CURNO 
      FROM DELPRICE, DELDET, DELHED
      WHERE DELPRICE.DPRDELRECNO = DELDET.DELRECNO
      AND DELDET.DELDLVORDNO =  DELHED.DLVORDNO
      AND DELPRICE.DPRRECNO = IN_DPRRECNO;     
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
          FT_PK_ERRORS.LOG_AND_CONTINUE;              
        WHEN OTHERS THEN
    ...
    BEGIN
      CURRENCYCONVERTTOBASE(V_DELNETTVALUE, V_CURNO, 1);
    EXCEPTION
        WHEN OTHERS THEN
          FT_PK_ERRORS.LOG_AND_STOP;              
    END;   
    ...
    BEGIN
      UPDATE DELPRICE
                SET DELEURONETTVAL = V_CALC_EUROVALUE,
                DELBASENETTVAL  = V_CALC_BASEVALUE,
                DELTOEURORATE   = V_EURORATE,
                DELTOBASERATE   = V_BASERATE
                WHERE DPRRECNO = IN_DPRRECNO
                AND NVL(DELINVSTATUS,0) < 10;   -- not invoiced                 
                COMMIT;
    EXCEPTION
    ...
    IF V_CONT = 1 THEN
        BEGIN
            SELECT ORDERS.ACTCSTCODE,  DELHED.DLVSTKLOC, DELHED.DLVSALOFFNO, DELDET.DELPRCPRDNO, DELHED.DLVDLTRECNO, DELPRICE.DELNETTVALUE, DELPRICE.DELFREEOFCHG,
                ...
                ORDERS.ORDCSTCODE, DELHED.DLVRELINV, DELHED.DLVTRANIND, DELDET.DELSTATUS, DELPRICE.DELINVSTATUS, DELHED.DLVCURRECNO 
            INTO V_ACTCSTCODE, V_DLVSTKLOC, V_DLVSALOFFNO, V_DELPRCPRDNO,V_DLVDLTRECNO, V_DELNETTVALUE, V_DELFREEOFCHG,
                V_DISCAMT,
                 V_ORCSTCODE, V_DLVRELINV, V_DLVTRANIND, V_DELSTATUS, V_DELINVSTATUS, V_CURNO
...
-- VERIFY THAT THIS IS A DELIVERY THAT WE CAN AMMEND
    ...
    BEGIN
      CURRENCYCONVERTTOBASE(V_VATAMOUNT, V_CURNO, 1);
    EXCEPTION
        WHEN OTHERS THEN
          FT_PK_ERRORS.LOG_AND_STOP;              
    END;
    ...
    -- TODO - WORK NEEDED HERE FOR THE EURO AND BASE  CALCULATIONS
        UPDATE DELPRICE SET
            ...
            DELEUROVATVALUE = V_CALC_EUROVALUE,
            DELBASEVATVALUE = V_CALC_BASEVALUE,
 ...
 PROCEDURE GET_DELAUDIT_FORAUTOCOST(LOWER_DELAUDRECNO DELAUDIT.DELAUDRECNO%TYPE, HIGHER_DELAUDRECNO DELAUDIT.DELAUDRECNO%TYPE) AS
        CURSOR DELAUDIT_TODO_CURSOR(LOWER_DELAUDRECNO DELAUDIT.DELAUDRECNO%TYPE, HIGHER_DELAUDRECNO DELAUDIT.DELAUDRECNO%TYPE) IS      
          (SELECT DELAUDIT.DPRRECNO DPRRECNO, 
            ...
            AND DELAUDIT.FORMNO <> 10009  -- PREINV DOES ITS OWN CALL TO AUTOCOSTS SO NO NEED TO INCLUDE IT 
    ...
    PROCEDURE CURRENCYCONVERTTOBASE(IN_VALUE  FLOAT, IN_CURRENCY  ACCCURRDESC.CURNO%TYPE, IN_CURRENCYUSEDFOR ACCCURRRATE.RATUSEFOR%TYPE) AS
    
    BEGIN
    
      V_CALC_EUROVALUE := IN_VALUE;
      V_CALC_BASEVALUE := IN_VALUE;
      
      GETBASEANDEURODECIMALPLACE;

      BEGIN                 
        SELECT RATRATETOEURO,  RATRATETOBASE
        INTO V_EURORATE , V_BASERATE        
        FROM ACCCURRRATE
        WHERE ACCCURRRATE.RATCURNO = IN_CURRENCY
        AND RATUSEFOR =  IN_CURRENCYUSEDFOR;
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20002, 'NO_DATA_FOUND) - VALUE' ||CHR(13) || CHR(10) || IN_VALUE);
          V_EURORATE := 1;
          V_BASERATE := 1 ;                          
        WHEN OTHERS THEN
              FT_PK_ERRORS.LOG_AND_CONTINUE;              
      ...
      BEGIN      
        V_CALC_EUROVALUE := IN_VALUE * V_EURORATE;
        V_CALC_EUROVALUE := ROUND(V_CALC_EUROVALUE, V_EURO_DECIMALPLACE);        
        
        V_CALC_BASEVALUE := IN_VALUE * V_BASERATE;
        V_CALC_BASEVALUE := ROUND(V_CALC_BASEVALUE, V_BASE_DECIMALPLACE);
      EXCEPTION
        WHEN OTHERS THEN
              FT_PK_ERRORS.LOG_AND_CONTINUE;              
      END;     
    END CURRENCYCONVERTTOBASE;
    
    
    PROCEDURE GETBASEANDEURODECIMALPLACE AS        
    BEGIN
      IF  NVL(V_BASE_DECIMALPLACE,0) < 1 THEN   
        BEGIN
            SELECT CURDECIMALPLACES INTO V_BASE_DECIMALPLACE FROM ACCCURRDESC WHERE CURNO = 1;
            SELECT CURDECIMALPLACES INTO V_EURO_DECIMALPLACE FROM ACCCURRDESC WHERE CURNO = 22;                  
        EXCEPTION
          WHEN OTHERS THEN
            V_BASE_DECIMALPLACE :=2;
            V_EURO_DECIMALPLACE :=2;
            FT_PK_ERRORS.LOG_AND_CONTINUE;              
        END;     
      END IF;    
    END GETBASEANDEURODECIMALPLACE;
