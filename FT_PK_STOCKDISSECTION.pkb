SET DEFINE OFF;
    ...
    ---*******************************************************************************************************************************************    
		...
		CALL FT_PK_ALLOCATE_CHECK.REPAIR_DELTOALL_MIN()
        ...
        -- OUR ALLOCATE LINE HAS SPLITS - NOW GET THE REQUIRED SPLIT QTYIES           
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    WHERE   (
                            (DELRECNO IS NULL) 
                            OR (DELRECNO IS NOT NULL AND  TO_DATE(NVL(SHIP_DATE, SYSDATE), 'DD/MM/YY')  <= TO_DATE(V_UPTODATE, 'DD/MM/YY'))
                            )                      
                    AND DELTOALL_SPLIT.ALLOCNO = VAR_USEALLOCNO;                
                END; 
                END IF;
            EXCEPTION             
                WHEN NO_DATA_FOUND THEN
