SET DEFINE OFF;
    ...
    AS
      cVersionControlNo   VARCHAR2(12) := '1.0.13'; -- Current Version Number
        ...
        -- AT THIS STAGE WE SHOULD HAVE A POSITIVE ALLOCATE NO  - NOW GET ITS DETAILS
            ...
            BEGIN
                ...
                INTO VAR_ALRDY_SPLITQTY_WGT, VAR_ALRDY_SPLITQTY_EACH, VAR_ALRDY_SPLITQTY_INNER
                    FROM ALLOCATE ORGALLOC , ALLOCATESPLITS LINKALLOCATE, ALLOCATE SPLITALLOCATE
                    WHERE ORGALLOC.ALLOCNO = LINKALLOCATE.BOXALLOCNO(+)
                    AND  LINKALLOCATE.SPLITALLOCNO = SPLITALLOCATE.ALLOCNO(+)
                    AND ORGALLOC.ALLOCNO = VAR_USEALLOCNO
        ...
        -- OUR ALLOCATE LINE HAS SPLITS - NOW GET THE REQUIRED SPLIT QTYIES
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    AND NVL(ALLFLAG,0) IN (V_ALLFLAG_OPEN, V_ALLFLAG_DIS);
                END;
                ELSE
                BEGIN
    ...
    -- NOW
        ...
        V_ALLFLAG_OPEN := 0 ; -- ALWAYS RESET THIS  TO 0 IN CASE IT WAS SET TO 1 IN PROCEDURE SET_FLAGTOIGNUNDISSINOVERALLOC
    ...
    ---*******************************************************************************************************************************************
    PROCEDURE DISSECTDELIVERY_SHOWERROR  (V_DLVORDNO IN NUMBER) IS
    BEGIN
      DISSECTDELIVERY(V_DLVORDNO);
      IF V_DISS_ERR IS NOT NULL THEN
        RAISE_APPLICATION_ERROR(-20002, V_DISS_ERR);
      END IF;
    END DISSECTDELIVERY_SHOWERROR;
    ...
    PROCEDURE DISSECTDELIVERY  (V_DLVORDNO IN NUMBER) IS
        ...
        BEGIN
            ...
            END;
            IF  VAR_NOOFERRS > 0  THEN
              V_DISS_ERR := 'SALES OFFICE PREFERENCE';
            END IF;
        END IF;
    ...
    PROCEDURE DISSECTDLV_WITHNOSOPREFCHK  (V_DLVORDNO IN NUMBER) IS
        ...
        IF VAR_NOOFERRS = 0 THEN
            BEGIN
              ...
              WHERE DELQTY > DALQTY;
              IF  VAR_NOOFERRS > 0  THEN
                V_DISS_ERR := 'DELDET.QTY <> DELTOALL.QTY';
              END IF;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    NULL;
        ...
        IF VAR_NOOFERRS = 0 THEN
            ...
            IF VAR_NOOFERRS > 0   THEN
            BEGIN
              IF V_ALLOWDISSFOROVERALLOC = 1     THEN
              BEGIN
                VAR_NOOFERRS :=CHKIFOVERALLOCATIONISALLOWED(V_DLVORDNO);  
              END;
              END IF;
            END;
            END IF;          
...
-- WHEN CHECKING IF AN DELIVERY CAN BE DISSECTED, YOU MAY  WISH TO RELAX THE OVERALLOCATE CHECK  SO THAT IF THERE IS ENOUGH AVAILBLE STOCK TO 
-- COVER THE DELIVERY THEN ALLOW IT TO BE DISSECTED  - THE OVER ALLOCATION IS CAUSED BY OTHER DELIVERIES 
-- THE WAY I AM DOING THIS IS TO HAVE A V_ALLFLAG_OPEN VARAIBLE THAT IS USED IN THE CHKISALLOCATE_OVERALLOC ROUTINE 
-- NORMALLY THIS IS SET TO 0 AND PICKS UP ALL NON-DISSECTED LINES 
-- IN THIS METHOD I CHANGE THIS TO -1 AND UPDATE THE DELIVERY THAT HAS AN OVERALLOC - 
-- CHKISALLOCATE_OVERALLOC WILL THEN ONLY PICK UP DELTOALLS WITH A ALLFLAG  IN (-1, 1)  AND SO WILL ONLY CHECK IF THIS DELIVERY & DISSECTED DELIVERIES MAKE THE LINE OVERALLOCATED

  PROCEDURE ALLOWDISSFOROVERALLOC IS
  BEGIN
    V_ALLOWDISSFOROVERALLOC := 1; 
  END ALLOWDISSFOROVERALLOC;
  
  FUNCTION CHKIFOVERALLOCATIONISALLOWED(V_DLVORDNO IN NUMBER) RETURN  NUMBER IS
            VAR_LCONT                  NUMBER(1) := 1;
            VAR_NOOFERRS                  NUMBER(10) := 0;
            
            -- CURSOR TO EXTRACT THE LIST OF OVERALLOCATED LINES ON THIS DELIVERY 
            CURSOR OVERALLOC_LINES_CURSOR(INS_DLVORDNO NUMBER) IS
                SELECT * FROM
                  (SELECT DALWIZUNIQUEID, FT_PK_STOCKDISSECTION.ISALLOCATE_OVERALLOC(NULL, DELTOALL.DALWIZUNIQUEID, DELHED.DLVSHPDATE)  ISALLOCATE_OVERALLOC
                  FROM DELTOALL, DELDET, DELHED
                  WHERE DELTOALL.DALTYPERECNO =  DELDET.DELRECNO
                  AND DELDET.DELDLVORDNO = DELHED.DLVORDNO 
                  AND DALRECORDTYPE = 1
                  AND DELDLVORDNO = INS_DLVORDNO)
                WHERE ISALLOCATE_OVERALLOC = 1;
                         
            OVERALLOC_LINES_RECORD OVERALLOC_LINES_CURSOR%ROWTYPE;
        ...
        BEGIN
          IF  V_ALLOWDISSFOROVERALLOC = 1 THEN
          BEGIN          
            -- THIS WILL PICK UP THE OVERALLOCATED LINES - THIS INCLUDES ALL ALLOCATED DELIVERIES
            OPEN  OVERALLOC_LINES_CURSOR(V_DLVORDNO);
            LOOP
              FETCH OVERALLOC_LINES_CURSOR INTO OVERALLOC_LINES_RECORD;
              EXIT WHEN OVERALLOC_LINES_CURSOR%NOTFOUND;

              -- SET THIS FLAG SO THAT ISALLOCATE_OVERALLOC FUNCTION DOES NOT PICK UP UNDISSECTED LINES            
              V_ALLFLAG_OPEN := -1; 
            
              -- AS THIS IS AN OVERALLOCATED LINE NOW SET THE ALLFLAG = -1 TEMPORARILY SO THAT ISALLOCATE_OVERALLOC IN THE NEXT QUERY INCLUDE THIS ONE IN THE ISALLOCATE_OVERALLOC FUNCTION
              IF VAR_LCONT = 1 THEN              
              BEGIN 
                BEGIN 
                  UPDATE DELTOALL SET ALLFLAG = V_ALLFLAG_OPEN WHERE DALWIZUNIQUEID = OVERALLOC_LINES_RECORD.DALWIZUNIQUEID AND NVL(ALLFLAG,0) = 0 ;
                  COMMIT;
                EXCEPTION
                  WHEN OTHERS THEN
                    FT_PK_ERRORS.LOG_AND_CONTINUE;
              ...
              -- AS V_ALLFLAG_OPEN IS NOW -1 ISALLOCATE_OVERALLOC WILL ONLY PICK UP THIS DELIVERY AND DISSECTED DELIVERIES              
              IF VAR_LCONT = 1 THEN              
              BEGIN 
                SELECT COUNT(*) NOOF INTO VAR_NOOFERRS
                FROM
                    ...
                    AND DELDET.DELDLVORDNO = DELHED.DLVORDNO 
                    AND DALRECORDTYPE = 1
                    AND DALWIZUNIQUEID = OVERALLOC_LINES_RECORD.DALWIZUNIQUEID)
                  WHERE ISALLOCATE_OVERALLOC = 1;
                  
                  -- SO IF IT IS STILL OVERALLOCATED THEN VAR_NOOFERRS >  0 AND WE DO NOT NEED TO CONTINUE
                  IF  VAR_NOOFERRS > 0 THEN
                  BEGIN
                    VAR_LCONT := 0;
                  END ;
              ...
              BEGIN 
              -- RESET THE DELTOALL - ALWAYS DO THIS REGARDLESS OF WHETHER OR NOT  THE ABOVE ROUTINES WORK
                UPDATE DELTOALL SET ALLFLAG = NULL WHERE DALWIZUNIQUEID = OVERALLOC_LINES_RECORD.DALWIZUNIQUEID AND ALLFLAG = V_ALLFLAG_OPEN;
                COMMIT;
              EXCEPTION
                WHEN OTHERS THEN
                  FT_PK_ERRORS.LOG_AND_CONTINUE;
        ...
        END ;          
        END IF;
        V_ALLFLAG_OPEN := 0;  -- RESET THIS FLAG 
      RETURN VAR_NOOFERRS ;
  END CHKIFOVERALLOCATIONISALLOWED;
