    ...
    PROCEDURE DISSECTDELIVERY  (V_DLVORDNO IN NUMBER) IS  
        ...
        IF VAR_NOOFERRS = 0 THEN  
            BEGIN      
              ...
              (SELECT DELRECNO, 
                      NVL(DELQTY, 0) DELQTY , 
                      NVL((SELECT SUM(NVL(DELTOALL.DALQTY,0)) FROM DELTOALL WHERE  DELTOALL.DALTYPERECNO =  DELDET.DELRECNO AND DALRECORDTYPE = 1 AND NVL(DELTOALL.DALQTY,0) > 0 ),0) DALQTY
              FROM DELDET 
