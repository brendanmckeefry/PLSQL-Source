...
create or replace PACKAGE BODY FT_COST_WRITES AS
  ...
  BEGIN
    ...
    POAUDFIL_INOUT.POAUDPERSON := FT_SESSION_UTILS.GET_FT_LOGONNO();
  ...
  BEGIN
    ...
    IF ABS(NEWLITPRDCOST - LOTITE_REC.LITPRDCOST) > 0.009 THEN
      UNITCOST_DIVISOR := CASE
                            WHEN LOTITE_REC.LITRCVCOMPLETE = CONST.C_YES AND NVL(LOTITE_REC.LITPURBYTYP, CONST.PERBOX) = CONST.PERBOX THEN LOTITE_REC.LITQTYRCV
                            WHEN NVL(LOTITE_REC.LITRCVCOMPLETE, CONST.C_NO) = CONST.C_NO AND NVL(LOTITE_REC.LITPURBYTYP, CONST.PERBOX) = CONST.PERBOX THEN LOTITE_REC.LITORGEXP
                            WHEN LOTITE_REC.LITPURBYTYP > CONST.PERBOX THEN LOTITE_REC.LITSTANDNOOF
                          END;

      IF NVL(UNITCOST_DIVISOR, 0.0) < 0.0001 THEN
        UNITCOST_DIVISOR := 1.0;
      END IF;

      UPDATE LOTITE
      SET LITUNICOST = ROUND((SELECT SUM(ITECHG.ICHRAWAPPAMT)
                              FROM ITECHG
                              WHERE ITECHG.CTYNO = CONST.CTYGOODS
                                AND ITECHG.ICHISTRECNO IS NULL
                                AND ITECHG.LITRECNO = LOTITE.LITITENO) / UNITCOST_DIVISOR, 4)
  ...
  BEGIN
    ...
    ELSIF ABS(ITECHG_INOUT.ICHRAWAPPAMT - ITECHG_REC.ICHRAWAPPAMT) > 0.009 THEN --Ignores NULLs
      ITECHG_REC.ICHRAWAPPAMT := ITECHG_INOUT.ICHRAWAPPAMT;
    ...
    ELSIF ABS(ITECHG_INOUT.ICHRAWAUTHAMM - NVL(ITECHG_REC.ICHRAWAUTHAMM, 0.0)) > 0.009 THEN --Ignores NULLs
      ITECHG_REC.ICHRAWAUTHAMM := ITECHG_INOUT.ICHRAWAUTHAMM;
    ...
    IF APP_CHNG THEN
      UPDATE EXPCHA
      SET (EXCCONAMM, EXCRAWAMM, EXCEUROAMM) = (SELECT SUM(ITECHG.ICHAPPAMT), SUM(ITECHG.ICHRAWAPPAMT), SUM(ITECHG.ICHRAWAPPAMT) * V_COSTS_REC.EXCTOEUROEXCRATE
                                                FROM ITECHG
    ...
    IF AUTH_CHNG THEN
      UPDATE EXPCHA
      SET (EXCAUTHCONAMM, EXCAUTHRAWAMM, EXCAUTHEUROAMM) = (SELECT SUM(ITECHG.ICHAUTHAMM), SUM(ITECHG.ICHRAWAUTHAMM), SUM(ITECHG.ICHRAWAUTHAMM) * V_COSTS_REC.EXCTOEUROEXCRATE
                                                            FROM ITECHG
                                                            WHERE ITECHG.EXCRECNO = EXPCHA.EXCCHAREC)
      WHERE EXCCHAREC = ITECHG_REC.EXCRECNO;
    END IF;
  ...
  IS
    ...
    L_DIVIDEND          FLOAT;
    L_DIVISOR           FLOAT;
    ITECHGS_TO_WRITE    FT_COST_WRITES.ITECHG_RECS := FT_COST_WRITES.ITECHG_RECS();
  ...
  BEGIN
    ...
    IF ABS(NEWRAWAMM_IN) < 0.0 THEN
      IF EXPCHA_REC.EXCAUTHRAWAMM < NEWRAWAMM_IN THEN
        FT_ERRORS.RAISE_ERROR(FT_ERRNUMS.FT_APPORTCOST);
      END IF;
    ELSE
      IF EXPCHA_REC.EXCAUTHRAWAMM > NEWRAWAMM_IN THEN
        FT_ERRORS.RAISE_ERROR(FT_ERRNUMS.FT_APPORTCOST);
    ...
    IF ABS(NEWRAWAMM_IN) > 0.009 THEN
      IF ABS(L_DIVISOR) < 0.0001 THEN
        FT_ERRORS.RAISE_ERROR(FT_ERRNUMS.FT_APPORTCOST);
      END IF;
    END IF;

    L_AMTLEFTTOAPP := NEWRAWAMM_IN;
    FOR ITR_REC IN APPORTION_PO_RECS(EXCCHAREC_IN) LOOP
      ...
      IF ABS(NEWRAWAMM_IN) < 0.01 THEN
        L_THISAMT := 0.0;
      ELSE
        L_THISAMT := ROUND(NEWRAWAMM_IN * (L_DIVIDEND / L_DIVISOR), 2);
      END IF;
    ...
    END LOOP;
  EXCEPTION
    WHEN FT_ERRNUMS.EXC_NONCRITICAL_ERROR THEN
      PARAMETER_LIST('#EXCCHAREC') := TO_CHAR(EXCCHAREC_IN);
      FT_ERRORS.RAISE_ERROR(FT_ERRNUMS.FT_APPORTCOST, PARAMETER_LIST);
    WHEN NO_DATA_FOUND THEN
      PARAMETER_LIST('#PARAMNAME') := 'EXCCHAREC_IN';
      PARAMETER_LIST('#PARAMVALUE') := TO_CHAR(EXCCHAREC_IN);
      FT_ERRORS.RAISE_ERROR(FT_ERRNUMS.FT_PARAMETER, PARAMETER_LIST);
