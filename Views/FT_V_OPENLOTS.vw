...
SELECT  LOTITE.LITITENO AS OPLITITENO,
        ...
        BALTOLOTS.RCVQTY - NVL(LAST_PROFITSE.OPTOTRCVD, 0) AS MOVTOTRCVD,
        NVL(SUM_SALES.DTLSALESVALUE, 0.0) AS OPGROSSSALES,
        NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(LAST_PROFITSE.OPGROSSSALES, 0.0) AS MOVGROSSSALES,
        NVL(SUM_SALES.DTLBULKSALESQTY, 0.0) AS OPSALESQTY,
        NVL(SUM_SALES.DTLBULKSALESQTY, 0.0) - NVL(LAST_PROFITSE.OPSALESQTY, 0.0)  AS MOVSALESQTY,
        NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(SUM_SALES_CHGS.SALESCOST, 0.0) AS OPNETTSALESVALUE,
        (NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(SUM_SALES_CHGS.SALESCOST, 0.0)) - NVL(LAST_PROFITSE.OPNETTSALESVALUE , 0.0) AS MOVNETTSALESVALUE,
        NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(SUM_SALES_CHGS.TOTALCOST, 0.0) AS OPPROFIT,
        NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(SUM_SALES_CHGS.TOTALCOST, 0.0) - NVL(LAST_PROFITSE.OPPROFIT , 0.0) AS MOVPROFIT,
        ...
        CASE
          ...
          ELSE ROUND((NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(SUM_SALES_CHGS.TOTALCOST, 0.0) - NVL(LAST_PROFITSE.OPPROFIT, 0.0)) / NULLIF((NVL(SUM_SALES.DTLSALESVALUE, 0.0) - NVL(SUM_SALES_CHGS.SALESCOST, 0.0)), 0.0) * 100.0, 2)
        ...
        NVL(REPORTED_PRICES.LOTRETURNQTY, 0) - NVL(LAST_PROFITSE.OPREPORTQTY, 0.0) AS MOVREPORTQTY,
        SUM_SALES.DTLOPENPRCQTY AS OPOPENPRCQTY,
        SUM_SALES.DTLOPENPRCQTY - NVL(LAST_PROFITSE.OPOPENPRCQTY, 0.0) AS MOVOPENPRCQTY,
        CASE 
          WHEN LOTPROFITSALOFF.REOPENED = 1 THEN NULL
          WHEN LOTITE.LITPAYTYP = 8 THEN (NVL(SUM_SALES_CHGS.TOTALCOST, 0.0) - NVL(SUM_SALES_CHGS.SUPREBATE, 0.0)) / NULLIF(SUM_SALES.DTLBULKSALESQTY, 0.0) 
        ...
        BALTOLOTS.ONSTOCKQTY - NVL(LAST_PROFITSE.OPCURQTY, 0) AS MOVCURQTY,
        NVL(SUM_PO_CHGS.TOTALCOST, 0.0) AS OPTOTCOST,
        NVL(SUM_PO_CHGS.TOTALCOST, 0.0) - NVL(LAST_PROFITSE.OPTOTCOST, 0.0) AS MOVTOTCOST,
        CASE WHEN LOTITE.LITPAYTYP = 8 THEN 0.0 ELSE NVL(SUM_PO_CHGS.STOCKVALUE, 0.0) END AS OPSTKVALUE,
        (CASE WHEN LOTITE.LITPAYTYP = 8 THEN 0.0 ELSE NVL(SUM_PO_CHGS.STOCKVALUE, 0.0) END) - NVL(LAST_PROFITSE.OPSTKVALUE, 0.0) AS MOVSTKVALUE,
        --NVL(SUM_PO_CHGS.GOODSCOST, 0.0) AS OPTOTALGDSCOST,
        --NVL(SUM_PO_CHGS.GOODSCOST, 0.0) - NVL(LAST_PROFITSE.OPTOTALGDSCOST, 0.0) AS MOVTOTALGDSCOST,
        CASE 
        ...
        CASE 
          ...
          WHEN LOTITE.LITPAYTYP = 8 THEN NVL(SUM_SALES_CHGS.GOODSCOST, 0.0) - NVL(SUM_PO_CHGS.GOODSAUTH, 0.0) + NVL(SUM_PO_CHGS.GOODSAUTH, 0.0)
          ELSE NVL(SUM_PO_CHGS.GOODSCOST, 0.0) 
        END - NVL(LAST_PROFITSE.OPTOTALGDSCOST, 0.0) AS MOVTOTALGDSCOST,
        NVL(SUM_PO_CHGS.GOODSAUTH, 0.0) AS OPAUTHGDSCOST,
        NVL(SUM_PO_CHGS.GOODSAUTH, 0.0) - NVL(LAST_PROFITSE.OPAUTHGDSCOST, 0.0) AS MOVAUTHGDSCOST,
        CASE 
        ...
        CASE 
          ...
          WHEN LOTITE.LITPAYTYP = 8 THEN NVL(SUM_SALES_CHGS.GOODSCOST, 0.0) - NVL(SUM_PO_CHGS.GOODSAUTH, 0.0) 
          ELSE NVL(SUM_PO_CHGS.GOODSCOST, 0.0) - NVL(SUM_PO_CHGS.GOODSAUTH, 0.0) 
        END - NVL(LAST_PROFITSE.OPGDSCOSTACCRUED, 0.0) AS MOVGDSCOSTACCRUED,
        (NVL(SUM_PO_CHGS.POCOST, 0.0) + NVL(SUM_SALES_CHGS.SUPREBATE, 0.0)) AS OPTOTALCHGCOST,
        (NVL(SUM_PO_CHGS.POCOST, 0.0) + NVL(SUM_SALES_CHGS.SUPREBATE, 0.0)) - NVL(LAST_PROFITSE.OPTOTALCHGCOST, 0.0) AS MOVTOTALCHGCOST,
        NVL(SUM_PO_CHGS.POCOSTAUTH, 0.0) AS OPAUTHCHGCOST,
        NVL(SUM_PO_CHGS.POCOSTAUTH, 0.0) - NVL(LAST_PROFITSE.OPAUTHCHGCOST, 0.0) AS MOVAUTHCHGCOST,
        NVL(SUM_PO_CHGS.POCOST, 0.0) + NVL(SUM_SALES_CHGS.SUPREBATE, 0.0) - NVL(SUM_PO_CHGS.POCOSTAUTH, 0.0) AS OPCHGCOSTACCRUED,
        (NVL(SUM_PO_CHGS.POCOST, 0.0) + NVL(SUM_SALES_CHGS.SUPREBATE, 0.0) - NVL(SUM_PO_CHGS.POCOSTAUTH, 0.0)) - NVL(LAST_PROFITSE.OPCHGCOSTACCRUED, 0.0) AS MOVCHGCOSTACCRUED,
        NVL(SUM_SALES_CHGS.SUPREBATE, 0.0) AS OPTOTSUPREBATE,
        NVL(SUM_SALES_CHGS.SUPREBATE, 0.0) - NVL(LAST_PROFITSE.OPTOTSUPREBATE, 0.0) AS MOVTOTSUPREBATE,
        NVL(SUM_PO_CHGS.SUPREBATEAUTH, 0.0) AS OPTOTAUTHSUPREBATE,
        NVL(SUM_PO_CHGS.SUPREBATEAUTH, 0.0) - NVL(LAST_PROFITSE.OPTOTAUTHSUPREBATE, 0.0) AS MOVTOTAUTHSUPREBATE,
        NVL(SUM_SALES_CHGS.SUPREBATE, 0.0) - NVL(SUM_PO_CHGS.SUPREBATEAUTH, 0.0)  AS OPSUPREBATEACCRUED,
        (NVL(SUM_SALES_CHGS.SUPREBATE, 0.0) - NVL(LAST_PROFITSE.OPTOTSUPREBATE, 0.0)) - (NVL(SUM_PO_CHGS.SUPREBATEAUTH, 0.0) - NVL(LAST_PROFITSE.OPTOTAUTHSUPREBATE, 0.0))  AS MOVSUPREBATEACCRUED,
        PURORD.PORRECNO,
        LOTHED.LHESUPREF,
        LOTITE.LITSTCLOC,
        LOTITE.LITRCVDATE
...
LEFT OUTER JOIN LAST_PROFITSE
  ON LAST_PROFITSE.OPLITITENO = BALTOLOTS.BTLLITITENO WITH READ ONLY;
