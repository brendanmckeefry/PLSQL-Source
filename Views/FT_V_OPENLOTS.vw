...
SUM_PO_CHGS AS ( 
  SELECT  BALTOLOTS.BTLLITITENO, 
          ...
          SUM(CASE WHEN BALTOLOTSCHGS.CTYNO = 1 THEN BALTOLOTSCHGS.BASEAUTHTOGLAMT ELSE 0.0 END) AS GOODSAUTH,
          ...
          SUM(CASE WHEN BALTOLOTSCHGS.CTYNO <> 1 THEN NVL(BALTOLOTSCHGS.BASEAUTHTOGLAMT,0) ELSE 0.0 END) AS POCOSTAUTH,
          SUM(NVL(BALTOLOTSCHGS.BASEAUTHTOGLAMT,0)) POCOSTAUTH,          
          ...
          SUM(CASE WHEN BALTOLOTSCHGS.CHARGECLASS = 4 THEN NVL(BALTOLOTSCHGS.BASEAUTHTOGLAMT,0) ELSE 0.0 END) AS SUPREBATEAUTH
  FROM BALTOLOTS
  INNER JOIN BALTOLOTSCHGS
    ON BALTOLOTSCHGS.BTLRECNO = BALTOLOTS.BTLRECNO
  /*LEFT OUTER JOIN 
    (SELECT ACCITE.AITITERECNO, 
            SUM(CASE WHEN ACCITE.AITDRCR = 'C' THEN -1 * ACCITE.AITBASEAMOUNT ELSE ACCITE.AITBASEAMOUNT END) AS AITBASEAMOUNT 
    FROM ACCITE 
    WHERE ACCITE.AITGLTRECNO > 0 
    GROUP BY ACCITE.AITITERECNO) AUTHTOGL
    ON AUTHTOGL.AITITERECNO = BALTOLOTSCHGS.ICHRECNO
  LEFT OUTER JOIN     
    (SELECT PORECOVITE.PORECOVAITITERECNO, 
          SUM(CASE WHEN PORECOVITE.PORECOVAITDRCR = 'C' THEN -1 * PORECOVITE.PORECOVAITBASEAMT ELSE PORECOVITE.PORECOVAITBASEAMT END) AS AITBASEAMOUNT 
      FROM PORECOVITE 
      WHERE PORECOVITE.PORECOVGLTRECNO > 0 
      AND PORECOVITE.PORECOVPSTRECNO = 34 
...
COMMENT ON TABLE FT_V_OPENLOTS  IS 'v1.0.2 - View to give open lot information';
