CREATE OR REPLACE PACKAGE BODY FT_PK_GOODS_COSTS AS
  ...
  IS
    ...
    WHERE (PO_VIEW.PROFITISED = CONST.C_FALSE OR PO_VIEW.REOPENED = CONST.C_TRUE)
  ...
  BEGIN
    ...
    IF GOODS_SALES_CUR%FOUND THEN
      ...
      L_SOLD_QTY := GOODS_SALES_REC.BULK_SALES_QTY; --- GOODS_SALES_REC.OPEN_PRICE deduction not required because the bulk sales already excludes the open price lines.
  ...
  BEGIN    
	...
	FT_PK_COSTING.AUTO_PO_COSTS(LITITENO_IN);--The AUTO_PO_COSTS is recalled to re-calculate any  % of cost accruals 
  EXCEPTION
    WHEN OTHERS THEN
      FT_PK_ERRORS.LOG_AND_STOP;
  END CALCULATEGOODSCOST;  
