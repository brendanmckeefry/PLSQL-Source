--
-- FT_PK_TRANS_COSTS  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY FT_PK_TRANS_COSTS AS
  ...
  cVersionControlNo   VARCHAR2(12) := '1.0.1'; -- Current Version Number
	...
	CURSOR TRANSFER_OWNER_TO_CUR(TRORECNO_TO_IN INTEGER)
	IS
	...
	CURSOR TRANSFER_OWNER_FROM_CUR(TRORECNO_FROM_IN INTEGER)
	IS
	SELECT TroRecNo, DelQty 
	  FROM Transferowner
	 WHERE TROFROMISTRECNO IN (SELECT TroToIstRecNo
	...
	SELECT ICHRECNO, EXCRECNO, ITECHG.DPRRECNO, ITECHG.DELRECNO, CTYNO, ICHAPPAMT, ICHAUTHAMM,
		   ...
		   ROUND((CASE WHEN NVL(TransferOwner.DelQty,0) > 0
		   			   THEN NVL(TransferOwner.DelQty,0) * 1.00 *  NVL(ICHAPPAMT,0) / NVL(Delprice.DelPrcQty,0) * 1.00
					   ELSE 0.00
				  END),2) TOTApp
	...
	ORDER BY EXCRECNO, DelRecNo, DPRRECNO;
...
-- 	TYPE COST_SCHEMA_RECS IS TABLE OF COST_SCHEMA_CUR%ROWTYPE INDEX BY PLS_INTEGER;
  ...
  PROCEDURE TRANSFERADDCHGSAPP_PRIV(EXPCHA_Collection_IN RECORD_NUMBERS)
	...
	--ValueToDo		  		 NUMBER;
  	...
  	BEGIN
		 FOR ITRREC IN COST_SCHEMA_CUR(EXPCHA_Collection) LOOP
			...
			FOR TRANSFER_TO_ITRREC IN TRANSFER_OWNER_TO_CUR(ITRREC.TRORECNO) LOOP
				...
				SELECT TRANSFER_TO_ITRREC.TRORECNO
	      	 		   BULK COLLECT INTO TRORECNO_Collection
				  FROM DUAL;
				  
				FOR TRANSFER_FROM_ITRREC IN TRANSFER_OWNER_FROM_CUR(TRANSFER_TO_ITRREC.TRORECNO) LOOP

					SELECT TRANSFER_FROM_ITRREC.TRORECNO
		      	 		   BULK COLLECT INTO TRORECNO_Collection
					  FROM DUAL;
  ...
  PROCEDURE TRANSFERADDCHGSAPP
	IS
	BEGIN
		 FOR DLVORDNO_ITR IN (SELECT DISTINCT SALES_VIEW.DLVORDNO FROM FT_V_DLV SALES_VIEW, AUTOCOSTS_PROCESS WHERE SALES_VIEW.DPRRECNO = AUTOCOSTS_PROCESS.DPRRECNO AND AUTOCOSTS_PROCESS.TRANSFERADDCHGSAPP = CONST.C_TRUE) LOOP
			 ...
			 SELECT EXPCHA.EXCCHAREC
			  ...
			  WHERE EXPCHA.EXCRTHNO IN (SELECT RTEHEAD.RTHNO
			  						   	  FROM RTEDETAI, RTEHEAD
										 WHERE RTEDETAI.RTDRTHRECNO = RTEHEAD.RTHNO
										   AND RTEDETAI.RTDDLVORDNO = DLVORDNO_ITR.DLVORDNO);
		END LOOP;
