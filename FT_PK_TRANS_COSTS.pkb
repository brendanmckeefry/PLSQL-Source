...
-- 	TYPE COST_SCHEMA_RECS IS TABLE OF COST_SCHEMA_CUR%ROWTYPE INDEX BY PLS_INTEGER;
  ...
  PROCEDURE TRANSFERADDCHGSAPP
	IS
	BEGIN
		 FOR DLVORDNO_ITR IN (SELECT DISTINCT SALES_VIEW.DLVORDNO FROM FT_V_DLV SALES_VIEW, AUTOCOSTS_DPRS WHERE SALES_VIEW.DPRRECNO = AUTOCOSTS_DPRS.DPRRECNO) LOOP
