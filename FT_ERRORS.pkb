...
CREATE OR REPLACE PACKAGE BODY FT_ERRORS AS 
  ...
  IS
    ...
    L_FTERRNO     FT_ERROR_CODES.FTERRORNO%TYPE  := FT_ERRNUMS.FT_NO_ERROR;
    L_START_POS   INTEGER;
  ...
  BEGIN
      ...
      EXCEPTION
        WHEN OTHERS THEN  
          L_FTERRNO := FT_ERRNUMS.FT_UNKNOWN;
      END;
  ...
  EXCEPTION 
    WHEN OTHERS THEN
      RAISE FT_ERRNUMS.EXC_UNHANDLED_ERROR;
      ROLLBACK;
  END LOGERROR;
  ...
  IS
    ...
    L_ORAERRNO              FT_ERROR_CODES.ORAERRNO%TYPE := FT_ERRNUMS.EN_GENERAL_ERROR;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        REC_FT_ERROR_CODES.FTERRORNO := FT_ERRNUMS.FT_UNKNOWN;
        REC_FT_ERROR_CODES.FTERRORCODE := FT_ERRNUMS.FT_UNKNOWN_CODE;
        REC_FT_ERROR_CODES.FTERRORMSG := FT_ERRNUMS.FT_UNKNOWN_MSG;
        REC_FT_ERROR_CODES.ORAERRNO := FT_ERRNUMS.FT_UNKNOWN_ORAERRNO;        
      WHEN OTHERS THEN
        LOG_AND_STOP();
    END;
    ...
    FT_STRING_UTILS.REPLACETOKENS(L_ERRMSG, TOKEN_LIST_IN);
    FT_STRING_UTILS.STRIPOUTCHAR(L_ERRMSG, C_DELIMTERCHAR);
  ...
  PROCEDURE RAISE_ERROR(FTERR_IN IN FT_ERROR_CODES.FTERRORNO%TYPE)
  IS
    EMPTY_TOKEN_LIST      FT_STRING_UTILS.TYPE_STRING_TOKENS;
  BEGIN
  ...
  PROCEDURE RAISE_ERROR(FTERR_IN IN FT_ERROR_CODES.FTERRORNO%TYPE, TOKEN_LIST_IN IN FT_STRING_UTILS.TYPE_STRING_TOKENS)
  IS
  BEGIN
  ...
  IS
    RET_FT_ERROR  FT_ERROR_CODES.FTERRORNO%TYPE := FT_ERRNUMS.FT_NO_ERROR;
    L_ORAMSG      VARCHAR2(2000) := SQLERRM;
  BEGIN
